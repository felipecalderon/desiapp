                    ...
                    <table className="table-auto">
                        ...
                        <tbody>
                            {orders.map((order) => {
                                ...
                                const cuotas =
                                    typeof order.startQuote === 'number' && typeof order.endQuote === 'number'
                                        ? `${order.startQuote} de ${order.endQuote}`
                                        : '-'
                                const pago =
                                    typeof order.startQuote === 'number' && typeof order.endQuote === 'number'
                                        ? Number(order.endQuote) !== Number(order.startQuote)
                                        : true
                                const esBodega = order.Store.isAdminStore
                                const estaCaducado = !esBodega && caduco && pago && order.status !== 'Pagado' && order.status !== 'Anulado'
                                const aunNoCaduca = !esBodega && pago && order.status !== 'Pagado' && order.status !== 'Anulado'
                                ...
                                const saldoPendiente =
                                    order.endQuote &&
                                    (order.endQuote && (total * 1.19) / order.endQuote) * (order.endQuote - order.startQuote)
                                return (
                                    <tr onClick={() => handleClickOrder(order.orderID)} className="cursor-pointer" key={order.orderID}>
                                        <td className="border px-2 py-2 hover:bg-slate-200">{order.Store.name}</td>
                                        <td className="border px-2 py-2 hover:bg-slate-200">{emision}</td>
                                        <td className="border px-2 py-2 hover:bg-slate-200 text-xs">{Type(order.type)}</td>
                                        <td className="border px-2 py-2 hover:bg-slate-200">{order.status}</td>
                                        <td className="border px-2 py-2 hover:bg-slate-200">{formatoPrecio(total * 1.19)}</td>
                                        <td className="border px-2 py-2 hover:bg-slate-200">{order.dte}</td>
                                        <td className="border px-2 py-2 hover:bg-slate-200">{cuotas}</td>
                                        <td className="border px-2 py-2 hover:bg-slate-200">{formatoPrecio(saldoPendiente)}</td>
