...
export default function Facturacion() {
    ...
    const handleClickOrder = (orderID: string) =>
        route.push(`/comprar/detalle/${orderID}`)
            ...
            <div className="flex flex-col justify-start items-center text-center gap-6 px-8 pt-10">
                <div className="overflow-x-auto">
                    <table className="table-auto">
                        ...
                        <tbody>
                            {orders.map((order) => {
                                const { fecha: emision } = getFecha(
                                    order.createdAt
                                )
                                const stringVto = calcularMesVto(
                                    order.expiration,
                                    order.startQuote || 0
                                )
                                const vencimiento = getFecha(stringVto)
                                const caduco = isCaducatedDate(
                                    vencimiento.fecha
                                )
                                const cuotas =
                                    typeof order.startQuote === 'number' &&
                                    typeof order.endQuote === 'number'
                                ...
                                if (estaCaducado)
                                    colorFondo = 'bg-red-300 hover:bg-red-500'
                                ...
                                return (
                                    <tr
                                        onClick={() =>
                                            handleClickOrder(order.orderID)
                                        }
                                        className="cursor-pointer dark:bg-blue-800"
                                        key={order.orderID}
                                    >
                                        <td className="border px-2 py-2 hover:bg-slate-200 dark:hover:bg-blue-700">
                                            {order.Store.name}
                                        </td>
                                        <td className="border px-2 py-2 hover:bg-slate-200 dark:hover:bg-blue-700">
                                            {emision}
                                        </td>
                                        <td className="border px-2 py-2 hover:bg-slate-200 dark:hover:bg-blue-700 text-xs">
                                            {Type(order.type)}
                                        </td>
                                        <td className="border px-2 py-2 hover:bg-slate-200 dark:hover:bg-blue-700">
                                            {order.status}
                                        ...
                                        <td
                                            className={`border px-2 py-2 ${colorFondo}`}
                                        >
                                            {vencimiento.fecha}
                                        </td>
                                    </tr>
