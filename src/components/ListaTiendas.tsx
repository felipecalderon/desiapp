                ...
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            ...
                            <th className="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sucursal</th>
                            <th className="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">RUT</th>
                            <th className="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Teléfono</th>
                            <th className="px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Gestores</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {stores.map((tienda, index) => (
                            ...
                            >
                                ...
                                <td className="px-2 text-sm italic py-4 max-w-[200px]">{tienda.address}</td>
                                <td className="px-2 text-sm italic py-4 max-w-[200px]">{<RoleVista role={tienda.role} />}</td>
                                <td className="px-2 text-sm italic py-4">{tienda.location}</td>
                                <td className="px-2 text-sm italic py-4">{tienda.rut}</td>
                                <td className="px-2 text-sm italic py-4">{tienda.phone}</td>
                                <td className="px-2 py-2">
                                    {tienda.Users && tienda.Users.length > 0
                                        ? tienda.Users?.map(({ name }) => (
                                              <p key={name} className="px-2 py-2 rounded-xl bg-slate-500 text-xs text-white text-center">
                                                  {name}
                                              </p>
                                          ))
                                        : '--'}
                                ...
                                <td className="px-2 py-2 relative">
                                    <div className="block relative space-x-2">
                                        <button
                                            className="bg-indigo-600 hover:bg-indigo-900 text-white px-2 py-2 rounded-xl text-xs"
                                            onClick={() => setEditingStore(tienda)}
                                        >
                                            Editar
                                        </button>
                                    </div>
                        ...
                        {editingStore && (
                            <div>
                                ...
                                <Input
                                    ...
                                    className="py-2"
                                />
                                {(editingStore.role === Role.Admin || editingStore.role === Role.Franquiciado) && (
                                    <div className="flex flex-row justify-end py-3">
                                        <Switch
                                            isSelected={editingStore.isAdminStore}
                                            onValueChange={() =>
                                                setEditingStore({ ...editingStore, isAdminStore: !editingStore.isAdminStore })
                                            }
                                            defaultSelected
                                            color="secondary"
                                        >
                                ...
                                <div className="flex flex-row justify-between">
                                    <Button
                                        className="bg-red-600 hover:bg-red-900 text-white px-2 py-2 rounded-xl text-xs"
                                        onClick={() => deletetienda(editingStore.storeID)}
                                    >
                                        {loading ? 'Borrando...' : 'Eliminar'}
                                    </Button>
                                    <Button
                                        disabled={loading}
                                        className="bg-green-600 hover:bg-green-900 text-white px-2 py-2 rounded-xl text-xs"
                                        onClick={saveEdit}
                                    >
