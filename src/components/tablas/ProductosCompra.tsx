        ...
        return (
            <>
                <div className="flex flex-row justify-between items-center">
                    <button
                        className="bg-blue-300 text-gray-900 font-semibold px-3 py-1 my-3 rounded-lg"
                        onClick={() => agregarCantidadesGlobales(products)}
                    >
                        Agregar 1 par a todos
                    </button>

                    <button
                        className="bg-red-300 text-gray-900 font-semibold px-3 py-1 my-3 rounded-lg"
                        onClick={() => agregarCantidadesGlobales(products, 0)}
                    >
                        Quitar todos los pares
                    </button>

                    <p>
                        Gestionando OC para: <span className="font-bold">{store?.name}</span> {store?.city}
                    </p>
                    <div className="bg-green-300 text-gray-900 italic px-3 py-1 my-3 rounded-lg">Markup: {store?.markup}</div>
                </div>
                <table className="min-w-full table-auto">
                    <thead className="sticky top-0 z-10">
                        <tr className="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                            ...
                            {user.role !== Role.Tercero && store?.role !== Role.Tercero && (
