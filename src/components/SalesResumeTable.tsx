...
const SalesResumeTable = () => {
    ...
    const redireccionVenta = (
        saleID: string,
        esOC: boolean | '' | undefined
    ) => {
                ...
                <TableBody>
                    {ventasFiltradas.map(
                        (
                            ...
                            const store =
                                stores &&
                                stores.find(({ storeID: ID }) => ID === storeID)
                            const esOC = type && type === 'OC'
                            const ventasOC = SaleProducts.reduce(
                                (acc, variacion) => {
                                    return (acc += variacion.quantityOrdered
                                        ? variacion.quantityOrdered
                                        : variacion.quantitySold)
                                },
                                0
                            )
                            const esUltimoDiaMes =
                                ...
                                new Date(createdAt).getMonth() !==
                                    new Date(
                                        ventasFiltradas[index + 1].createdAt
                                    ).getMonth()
                            const noEsDelFinal =
                                index !== ventasFiltradas.length - 1 &&
                                esUltimoDiaMes
                            return (
                                ...
                                >
                                    ...
                                    <TableCell>
                                        ...
                                        {ventasOC < 3 ? (
                                            <p>
                                                [T{SaleProducts[0].sizeNumber}]{' '}
                                                {SaleProducts[0].quantitySold}x{' '}
                                                {SaleProducts[0].name}{' '}
                                                {ventasOC > 1 && (
