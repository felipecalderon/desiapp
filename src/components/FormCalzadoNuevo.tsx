...
const CrearCalzado = () => {
    ...
    useEffect(() => {
        const { parentProductID, priceList, sizeNumber, sku, stockQuantity } = formBody
            ...
            {showForm && (
                <form className="mt-4 bg-slate-200 px-6" onSubmit={onSubmitForm} onChange={handleFormChange}>
                    ...
                    {product && (
                        <div className="flex items-center mb-3 gap-1">
                            ...
                            {product.ProductVariations.map(({ sizeNumber, variationID }) => {
                                return (
                                    <div key={variationID} className="bg-yellow-300 rounded-full w-8 h-8 flex justify-center items-center">
                    ...
                    {formStatus.priceList && (
                        <p className="-mt-1 mb-2 text-xs italic">Ingresa Precio Plaza: {formatoPrecio(formBody.priceList)}</p>
                    )}
