...
const ActualizarCalzado = () => {
    ...
    useEffect(() => {
        if (products) {
            const filter = products.find(({ productID }) => productID === formBody.parentProductID)
            if (filter) {
                const variation = filter.ProductVariations.find((variation) => variation.sku === showForm?.sku)
                if (variation) {
                    setFormBody((formBody) => ({
                        ...formBody,
                        priceList: variation.priceList,
                        stockQuantity: variation.stockQuantity,
                        markup: formBody.markup,
                    }))
                }
            }
        }
    ...
    useEffect(() => {
        const { markup, priceList, stockQuantity } = formBody

        const esValido = validateMarkup(markup) && validatePriceList(priceList) && validateStockQuantity(stockQuantity)
    ...
    return (
        <div>
            <button className="flex items-center justify-center mx-auto text-white rounded-md px-16 my-3 bg-blue-700 h-10 hover:bg-blue-800 transition duration-300" onClick={toggleForm}>
            ...
            {showSize && (
                <form className="mt-4 bg-slate-200 px-6" onSubmit={onSubmitForm} onChange={handleFormChange}>
                    ...
                    <select name="parentProductID" className="block w-full p-2 mb-2 border rounded-md">
                    ...
                    {showForm && (
                        <div>
                            <div className="flex flex-row justify-start items-center gap-3">
                                <div className="bg-orange-600 cursor-pointer py-2 px-3 text-white text-xs rounded-lg" onClick={() => setDeleteSize(!deleteSize)}>
                                    Eliminar talla {showForm.sizeNumber}
                                </div>
                                {deleteSize && (
                                    <div className="bg-red-700 cursor-pointer py-2 px-3 text-white text-xs rounded-lg" onClick={() => onDeleteVariant(showForm.sku)}>
                            ...
                            <input value={formBody.priceList} onFocus={handleFocus} onBlur={handleBlur} autoComplete="off" type="number" name="priceList" placeholder="Precio Plaza" className="block w-full p-2 my-2 border rounded-md" />
                            {formStatus.priceList && <p className="-mt-1 mb-2 text-xs italic">Precio Plaza Actual: {formatoPrecio(showForm.priceList)}</p>}

                            <input value={formBody.stockQuantity} onFocus={handleFocus} onBlur={handleBlur} autoComplete="off" type="number" name="stockQuantity" placeholder="Stock total" min={0} max={500} className="block w-full p-2 my-2 border rounded-md" />
                            {formStatus.stockQuantity && <p className="-mt-1 mb-2 text-xs italic">Stock central actual: {showForm.stockQuantity}</p>}

                            <input value="1.8" type="hidden" name="markup" placeholder="Markup costo Producto" step="0.1" min={0.5} max={10} className="block w-full p-2 my-2 border rounded-md" />

                            <button className={`flex items-center justify-center ${!validForm && 'cursor-not-allowed'} text-white rounded-md px-6 my-2 bg-blue-600 h-10 hover:bg-blue-800 transition duration-300`}>Actualizar calzado</button>
                        </div>
                    )}
                </form>
            )}
