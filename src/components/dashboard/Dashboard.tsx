...
const DashBoard = () => {
    ...
    const { filteredSales, filterMonth, filterYear, orders } = storeSales()
    ...
    console.log(orders)
    const { tiendasPropias } = ventasFiltradas.reduce(
        (acc, sale) => {
            ...
            if (estaVendido && sonTiendasPropias)
                return {
                    ...acc,
                    tiendasPropias: {
    ...
    const { tiendasTerceros } = orders.reduce(
        (acc, sale) => {
            const estaVendido = sale.status === 'Pagado' || sale.status === 'Enviado'
            const paresVendidos = sale.ProductVariations.reduce((acc, variation) => {
                if (variation.quantityOrdered) {
                    return acc + variation.quantityOrdered
                }
                return acc + variation.quantityOrdered
            ...
            if (estaVendido)
                return {
                    ...acc,
                    tiendasTerceros: {
                        ventas: acc.tiendasTerceros.ventas + Number(sale.total),
    ...
    if (store && store.role === Role.Tercero)
        return (
            <>
    ...
    const totalVentas = tiendasPropias.ventas + tiendasTerceros.ventas
    const totalPares = tiendasPropias.pares + tiendasTerceros.pares
                ...
                <div className="flex flex-col justify-center items-center p-20 gap-3 rounded-3xl">
                    ...
                    <div className="text-center">
                        <div className="flex gap-3 justify-center mb-6">
                            <CardDataSale icon={PiTreeStructureFill} title="Tiendas Propias">
                                <ul>
                                    <li>
                                        <span className="font-bold text-blue-800">{tiendasPropias.pares}</span> Pares vendidos
                                    </li>
                                    <li>
                                        <span className="font-bold text-blue-800">{formatoPrecio(tiendasPropias.ventas / 1.19)}</span> + IVA
                                    </li>
                                </ul>
                            </CardDataSale>
                            <CardDataSale icon={IoStorefront} title="Tiendas Terceros">
                                <ul>
                                    <li>
                                        <span className="font-bold text-blue-800">{tiendasTerceros.pares}</span> Pares vendidos
                                    </li>
                                    <li>
                                        <span className="font-bold text-blue-800">{formatoPrecio(tiendasTerceros.ventas / 1.19)}</span> +
                                        IVA
                                    </li>
                                </ul>
                            </CardDataSale>
                            <CardDataSale icon={FaLayerGroup} title="Totales">
                                <ul>
                                    <li>
                                        <span className="font-bold text-blue-800">{totalPares}</span> Pares vendidos
                                    </li>
                                    <li>
                                        <span className="font-bold text-blue-800">{formatoPrecio(totalVentas / 1.19)}</span> + IVA
                                    </li>
                                </ul>
                            </CardDataSale>
                            <CardDataSale icon={FiTable} title="Resumen">
                                <h2 className="max-w-[150px]">
                                    <span className="font-bold text-blue-800">{tiendasQueHanVendido}</span> de{' '}
                                    <span className="font-bold text-blue-800">{totalStores.length}</span> tiendas han realizado ventas
                                </h2>
                            </CardDataSale>
                        </div>
                        <div className="flex gap-2 justify-center">
                            {tiendasQueHanVendido > 0 &&
                                totalStores.length &&
                    ...
                    <div className="flex flex-col items-center">
                        <div className="text-xl italic mb-3 text-blue-500">
                            {filterMonth && `Filtrando por fecha: ${filterMonth}`} {filterYear && filterYear}
                        </div>
                        <SalesResumeTable />
                    </div>
                </div>
    ...
    return (
        <>
            <div className="flex flex-col justify-center items-center p-16 gap-8 rounded-3xl">
                ...
                <div className="text-center">
                    <h2 className="text-lg font-semibold whitespace-pre">VENTAS DEL MES: {totalPares}</h2>
                    <h2 className="text-lg font-semibold">TOTAL: {formatoPrecio(totalVentas)} IVA. INC.</h2>
                </div>
                <div className="flex flex-col items-center">
